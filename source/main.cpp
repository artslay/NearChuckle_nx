...
static bool patchFarCryDisplayInfoDefault(LoadedSo* so, uint8_t* stage_base,
    ...
    // Far Cry/CryEngine 1 release builds create this CVar as:
    // CreateVariable("r_DisplayInfo", "0", ...).
    // The command-line value can therefore be overwritten when the CVar is
    // created. Locate the exact adjacent string pair and make its default "1",
    // matching the debug build and the requested startup behaviour.
    ...
    const char old_value = static_cast<char>(*default_value);
    *default_value = static_cast<uint8_t>('1');

    compatLogFmt("FARCRY DISPLAYINFO PATCH: r_DisplayInfo default '%c' -> '1'",
                 old_value);
    return true;
}
