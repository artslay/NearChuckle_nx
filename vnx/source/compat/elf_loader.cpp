...
extern "C" bool compatSaveFarCryProfileConfiguration(const char* profile) {
    ...
    void* game = getIGame(system);
    if (!game)
        return false;

    void*** gameVtable = reinterpret_cast<void***>(game);
    ...
    compatLogFmt("PROFILE SAVE: engine IGame::SaveConfiguration profile=%s",
                 profile);

    saveConfiguration(game, "system.cfg", "game.cfg", profile);

    compatLogFmt("PROFILE SAVE: engine serializer returned profile=%s",
                 profile);
    return true;
}
