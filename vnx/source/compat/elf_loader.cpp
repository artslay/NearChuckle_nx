...
extern "C" bool compatGetFarCryProfile(char* out, size_t outSize) {
    ...
    const char* value = getString(cvar);
    if (!value)
        return false;
...
extern "C" bool compatWriteFarCryProfileSystemConfig(const char* profile) {
    ...
    const auto setString =
        reinterpret_cast<SetStringFn>((*profileVtable)[4]);
    if (!getString || !setString)
        return false;

    const char* before = getString(profileCvar);
    if (!before || std::strcmp(before, profile) != 0) {
        setString(profileCvar, profile);
        const char* after = getString(profileCvar);
        if (!after || std::strcmp(after, profile) != 0)
            return false;
    }
    ...
    std::fputs("-- [System-Configuration]\r\n", file);
    std::fputs("-- Attention: This file is generated by the system, do not modify! Editing is not recommended! \r\n\r\n",
               file);
    ...
    struct DumpSink {
        ...
        static void writeEscaped(FILE* out, const char* value) {
            if (!value)
                return;
        ...
        virtual void OnElementFound(void* cvar) {
            if (!cvar)
                return;
            ...
            if (!getFlags || !getString || !getName)
                return;
            if (!(getFlags(cvar) & 0x00000100))
                return; // VF_DUMPTODISK

            const char* name = getName(cvar);
            const char* value = getString(cvar);
            if (!name || !*name || !value)
                return;

            if (std::strcmp(name, "g_playerprofile") == 0)
                wroteProfile = true;

            std::fputs(name, file);
            std::fputs(" = \"", file);
            writeEscaped(file, value);
            std::fputs("\"\r\n", file);
        }
    };
    ...
    DumpSink sink;
    sink.file = file;
    sink.wroteProfile = false;

    using DumpCVarsFn = void (*)(void*, void*, unsigned int);
    const auto dumpCVars =
        reinterpret_cast<DumpCVarsFn>((*consoleVtable)[12]);
    if (!dumpCVars) {
        std::fclose(file);
        return false;
    }

    dumpCVars(console, &sink, 0);

    if (!sink.wroteProfile)
        std::fprintf(file, "g_playerprofile = \"%s\"\r\n", profile);

    std::fclose(file);

    compatLogFmt("PROFILE SYSTEM CFG: wrote full DumpCVars -> %s", path);
    return true;
}
